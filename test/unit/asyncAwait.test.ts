...
async function extract(iterator: TarIterator, dest: string, options: ExtractOptions & { concurrency?: number }) {
  const links: Entry[] = [];
...
async function extractForEach(iterator: TarIterator, dest: string, options: ExtractOptions & { concurrency?: number }) {
  const links: Entry[] = [];
